...
namespace Miningcore.Crypto.Hashing.Progpow.Meraki;
...
public class Cache : IProgpowCache
{
    ...
    public void Dispose()
    {
        ...
        {
            MerakiPow.DestroyContext(handle);
            handle = IntPtr.Zero;
        }
    }
    ...
    public unsafe bool Compute(ILogger logger, int blockNumber, byte[] hash, ulong nonce, out byte[] mixDigest, out byte[] result)
    {
        ...
        var value = new MerakiPow.Ethash_result();

        var inputHash = new MerakiPow.Ethash_hash256();
        inputHash.bytes = hash;

        fixed(byte* input = hash)
        {
            value = MerakiPow.hash(handle, blockNumber, ref inputHash, nonce);
        }

        if(value.final_hash.bytes == null)
        {
            logger.Error(() => $"Meraki hash returned null");
            return false;
        }
        ...
        messageBus?.SendTelemetry("Meraki", TelemetryCategory.Hash, sw.Elapsed, true);
