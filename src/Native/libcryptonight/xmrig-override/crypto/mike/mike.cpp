...
static const core_hash_func core_hash[11] = { h0, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10 };
...
void benchmark()
{
    ...
    std::thread t([]() {
        ...
        LOG_VERBOSE("Running Mike benchmark on logical CPUs %u and %u (max scratchpad size %zu MB, huge pages %s)", thread_index1, thread_index2, max_scratchpad_size >> 20, memory->isHugePages() ? "on" : "off");
    ...
    LOG_VERBOSE("---------------------------------------------");
    LOG_VERBOSE("|         Mike tuning results         |");
    LOG_VERBOSE("---------------------------------------------");
...
void hash_octa(const uint8_t* data, size_t size, uint8_t* output, cryptonight_ctx** ctx, HelperThread* helper, bool verbose)
{
    ...
    if (verbose) {
        ...
        if (memcmp(cn_indices, prev_indices, sizeof(prev_indices)) != 0) {
            ...
            for (int i = 0; i < 3; ++i) {
                LOG_INFO("%s Mike algo %d: %s", Tags::cpu(), i + 1, cn_names[cn_indices[i]]);
            }
        }
    }
...
void hash(const uint8_t* data, size_t size, uint8_t* output, cryptonight_ctx** ctx, HelperThread*, bool verbose)
{
    ...
    uint8_t tmp[64 * N];
    const uint64_t core_indices_per_part[] = { 5, 5, 1 };

    for (uint64_t part = 0; part < 3; ++part) {
        uint64_t num_indices = core_indices_per_part[part];
